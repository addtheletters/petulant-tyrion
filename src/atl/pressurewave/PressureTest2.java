...
public class PressureTest2 {
	...
	private boolean balanceWithNeighbors(int x, int y, int[][] gc) {
		double neighborBalanceCoefficient = 0;
		return false;
	}

	
	private List<Neighbor> getNeighbors(int x, int y){
		ArrayList<Neighbor> neighs = new ArrayList<Neighbor>();
		neighs.add(new Neighbor(x+1, y));
		neighs.add(new Neighbor(x, y+1));
		neighs.add(new Neighbor(x-1, y));
		neighs.add(new Neighbor(x, y-1));
		return neighs;
	}
	...
	private void input() {
		if (mouseEnabled) {
			...
			if (Mouse.isButtonDown(0)) {
				...
				setValue(mouseX, mouseY, grid[mouseX][mouseY] + MOUSE_POWER, grid);
			}
	...
	public class Neighbor{
		...
		public Neighbor(int x, int y, double balanceVal){
			xpos = x;
			ypos = y;
